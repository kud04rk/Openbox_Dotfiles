...
-- Compatibility shim: the shared-script-property API was removed from mpv
-- (it lived under mp.utils, later mp.*). It only lets other scripts observe
-- OSC visibility, so no-op gracefully when unavailable.
local function set_shared_script_property(name, value)
    if mp.shared_script_property_set then
        mp.shared_script_property_set(name, value)
    elseif utils.shared_script_property_set then
        utils.shared_script_property_set(name, value)
    end
end
...
local user_opts = {
    ...
    livemarkers = true,        -- update chapter markers on streams whose duration changes
...
function visibility_mode(mode, no_osd)
    ...
    set_shared_script_property("osc-visibility", mode)
